...
TPL void compute_shape_heterogeneity(
    ...
    for (index_t e = 0; e < num_edges; ++e) {
        ...
        real_t comp1 = (eigs_u[0] + eigs_u[1] + eigs_u[2]) / 3.0 / std::cbrt(n1);
        real_t comp2 = (eigs_v[0] + eigs_v[1] + eigs_v[2]) / 3.0 / std::cbrt(n2);
        real_t compm = (eigs_merged[0] + eigs_merged[1] + eigs_merged[2]) / 3.0 / std::cbrt(n1 + n2);
